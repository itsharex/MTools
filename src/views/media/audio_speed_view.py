...
class AudioSpeedView(ft.Container):
    ...
    def _build_ui(self) -> None:
        ...
        header = ft.Row(
            controls=[
                ...
                ft.Text("音频倍速调整", size=28, weight=ft.FontWeight.BOLD),
            ],
                    ...
                    ft.Container(
                        content=ft.Row(
                            controls=[
                                ...
                                ft.Text(
                                    "支持格式: MP3, WAV, AAC, FLAC, OGG, M4A, WMA, APE 等",
                                    size=11,
        ...
        format_options = ft.Container(
            content=ft.Column(
                controls=[
                    ft.Text("输出格式", size=18, weight=ft.FontWeight.W_600),
                    ft.Container(
    ...
    def _init_empty_state(self) -> None:
        ...
        self.file_list_view.controls.append(
            ft.Container(
                content=ft.Column(
                    controls=[
                        ft.Icon(ft.Icons.AUDIOTRACK, size=48, color=ft.Colors.ON_SURFACE_VARIANT),
            ...
            for idx, file_path in enumerate(self.selected_files):
                ...
                self.file_list_view.controls.append(
                    ft.Container(
                        content=ft.Row(
                            controls=[
                                ft.Icon(ft.Icons.AUDIOTRACK, size=20, color=ft.Colors.PRIMARY),
                                ft.Column(
                                    controls=[
    ...
    def _on_process(self, e: ft.ControlEvent) -> None:
        """开始处理音频。"""
        if not self.selected_files:
            self._show_message("请先选择要处理的音频", ft.Colors.ORANGE)
            return
